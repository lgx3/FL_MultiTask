...
def main():
    ...
    dict_users_fmnist, distribution_fmnist, class_weight_fmnist = exchangeDistribution(dict_users_fmnist,
                                                                                       distribution_fmnist, 'fmnist')
    dict_users_cifar10, distribution_cifar10, class_weight_cifar10 = exchangeDistribution(dict_users_cifar10,
                                                                                          distribution_cifar10,
                                                                                          'cifar10')
    ...
    for Budget_Task in Task_Budget:
        ...
        print("\n*******************************************背包贪婪选择客户端*******************************************")
        Knapsack_greedy_client_selected, Knapsack_greedy_payment = Individual_Knapsack_greedy(budget_q_2, client_dict,
                                                                                              Budget_Task)
        ...
        for task in Task:
            ...
            if task == 0:
                acc_test, acc_all_mnist = fl_training(client_selected_every_task_q[task], dict_users_mnist, p,
                                                      dataset_train, dataset_test, args, dataset_name[task])
                print("EMD-MQFL选择时，mnist选中客户端参与训练的准确率：{}".format(acc_all_mnist))
                EMD_mqfl_acc.append(acc_all_mnist)

                BidPrice_first_acc_test, BidPrice_first_acc_all_mnist = fl_training(Bidfirst_client_selected[task],
                                                                                    dict_users_mnist, p, dataset_train,
                                                                                    dataset_test, args,
                                                                                    dataset_name[task])
                ...
                Knapsack_greedy_acc_test, Knapsack_greedy_acc_all_mnist = fl_training(
                    Knapsack_greedy_client_selected[task], dict_users_mnist, p, dataset_train, dataset_test, args,
                    dataset_name[task])
                ...
                Random_acc_test, Random_acc_all_mnist = fl_training(Random_client_selected[task], dict_users_mnist, p,
                                                                    dataset_train, dataset_test, args,
                                                                    dataset_name[task])
                print("随机选择时，mnist选中客户端参与训练的准确率：{}".format(Random_acc_all_mnist))
                Random_acc.append(Random_acc_all_mnist)

            if task == 1:
                acc_test, acc_all_fmnist = fl_training(client_selected_every_task_q[task], dict_users_fmnist, p,
                                                       dataset_train, dataset_test, args, dataset_name[task])
                print("EMD-MQFL选择时，fmnist选中客户端参与训练的准确率：{}".format(acc_all_fmnist))
                EMD_mqfl_acc.append(acc_all_fmnist)

                BidPrice_first_acc_test, BidPrice_first_acc_all_fmnist = fl_training(Bidfirst_client_selected[task],
                                                                                     dict_users_fmnist, p,
                                                                                     dataset_train, dataset_test, args,
                                                                                     dataset_name[task])
                ...
                Knapsack_greedy_acc_test, Knapsack_greedy_acc_all_fmnist = fl_training(
                    Knapsack_greedy_client_selected[task], dict_users_fmnist, p, dataset_train, dataset_test, args,
                    dataset_name[task])
                ...
                Random_acc_test, Random_acc_all_fmnist = fl_training(Random_client_selected[task], dict_users_fmnist, p,
                                                                     dataset_train, dataset_test, args,
                                                                     dataset_name[task])
            ...
            if task == 2:
                acc_test, acc_all_cifar10 = fl_training(client_selected_every_task_q[task], dict_users_cifar10, p,
                                                        dataset_train, dataset_test, args, dataset_name[task])
                print("EMD-MQFL选择时，cifar10选中客户端参与训练的准确率：{}".format(acc_all_cifar10))
                EMD_mqfl_acc.append(acc_all_cifar10)

                BidPrice_first_acc_test, BidPrice_first_acc_all_cifar10 = fl_training(Bidfirst_client_selected[task],
                                                                                      dict_users_cifar10, p,
                                                                                      dataset_train, dataset_test, args,
                                                                                      dataset_name[task])
                ...
                Knapsack_greedy_acc_test, Knapsack_greedy_acc_all_cifar10 = fl_training(
                    Knapsack_greedy_client_selected[task], dict_users_cifar10, p, dataset_train, dataset_test, args,
                    dataset_name[task])
                ...
                Random_acc_test, Random_acc_all_cifar10 = fl_training(Random_client_selected[task], dict_users_cifar10,
                                                                      p, dataset_train, dataset_test, args,
                                                                      dataset_name[task])
        ...
        for iter in range(args.epochs):
            ...
            if iter == 0:
                ...
                for task in Task:
                    ...
                    if task == 0:
                        acc_test_mnist, loss_locals_mnist, loss_test_mnist, w_global_mnist = one_epoch_fl_training(Random_client_selected[task], dict_users_mnist, p, dataset_train, dataset_test, args,
                            dataset_name[task], iter)
                    ...
                    if task == 1:
                        acc_test_fmnist, loss_locals_fmnist, loss_test_fmnist, w_global_fmnist = one_epoch_fl_training(Random_client_selected[task], dict_users_fmnist, p, dataset_train, dataset_test, args,
                            dataset_name[task], iter)
                    ...
                    if task == 2:
                        acc_test_cifar10, loss_locals_cifar10, loss_test_cifar10, w_global_cifar10 = one_epoch_fl_training(Random_client_selected[task], dict_users_cifar10, p, dataset_train, dataset_test, args,
                            dataset_name[task], iter)
            ...
            if iter != 0:
                print(
                    "------------------------------------------开始第{}轮训练------------------------------------------".format(
                        iter))
                # (1)获取质量
                Quality_all = FairGetQuality(client_selected_new, loss_start_task, loss_end_task_client, client_dict,
                                             Quality_all, iter)
                ...
                for task in Task:
                    ...
                    if task == 0:
                        ...
                        acc_test_mnist, loss_locals_mnist, loss_test_mnist, w_global_mnist = one_epoch_fl_training(
                            client_selected_new[task], dict_users_mnist, p, dataset_train, dataset_test, args,
                            dataset_name[task], iter, Global_model_set[iter - 1][task])
                    ...
                    if task == 1:
                        ...
                        acc_test_fmnist, loss_locals_fmnist, loss_test_fmnist, w_global_fmnist = one_epoch_fl_training(
                            client_selected_new[task], dict_users_fmnist, p, dataset_train, dataset_test, args,
                            dataset_name[task], iter, Global_model_set[iter - 1][task])
                        if iter != (args.epochs - 1):
                            loss_start_task[iter + 1][task] = loss_test_fmnist
                        # print("fmnist训练本地损失：{}".format(loss_locals_fmnist))
                    ...
                    if task == 2:
                        ...
                        acc_test_cifar10, loss_locals_cifar10, loss_test_cifar10, w_global_cifar10 = one_epoch_fl_training(
                            client_selected_new[task], dict_users_cifar10, p, dataset_train, dataset_test, args,
                            dataset_name[task], iter, Global_model_set[iter - 1][task])
                        if iter != (args.epochs - 1):
                            loss_start_task[iter + 1][task] = loss_test_cifar10
                        # print("cifar10训练本地损失：{}".format(loss_locals_cifar10))
    ...
    for budget_i in range(len(Task_Budget_sum)):
        avg_max_Acc_EMD_MQFL.append(
            (max_Acc_EMD_MQFL[0][budget_i] + max_Acc_EMD_MQFL[1][budget_i] + max_Acc_EMD_MQFL[2][budget_i]) / 3)
        avg_max_Acc_Bid_Price_First.append((max_Acc_Bid_Price_First[0][budget_i] + max_Acc_Bid_Price_First[1][
            budget_i] + max_Acc_Bid_Price_First[2][budget_i]) / 3)
        avg_max_Acc_Knapsack_greedy.append((max_Acc_Knapsack_greedy[0][budget_i] + max_Acc_Knapsack_greedy[1][
            budget_i] + max_Acc_Knapsack_greedy[2][budget_i]) / 3)
        avg_max_Acc_FAIR.append((max_Acc_FAIR[0][budget_i] + max_Acc_FAIR[1][budget_i] + max_Acc_FAIR[2][budget_i]) / 3)
        avg_max_Acc_Random.append(
            (max_Acc_Random[0][budget_i] + max_Acc_Random[1][budget_i] + max_Acc_Random[2][budget_i]) / 3)
    ...
    plt.grid()
    plt.legend()
    plt.savefig('D:/Code/WorkPlace/pythonWork/FL_MultiTask/save/Different_Budget_EMD_Random_comparsion_FederatedLearning/20230410_实验1_0.8_8_TaskQuality.png')
    plt.show()
    ...
    plt.legend()
    plt.savefig(
        'D:/Code/WorkPlace/pythonWork/FL_MultiTask/save/Different_Budget_EMD_Random_comparsion_FederatedLearning/20230410_实验1_0.8_8_mnist.png')
    plt.show()
    ...
    plt.legend()
    plt.savefig(
        'D:/Code/WorkPlace/pythonWork/FL_MultiTask/save/Different_Budget_EMD_Random_comparsion_FederatedLearning/20230410_实验1_0.8_8_fmnist.png')
    plt.show()
    ...
    plt.legend()
    plt.savefig(
        'D:/Code/WorkPlace/pythonWork/FL_MultiTask/save/Different_Budget_EMD_Random_comparsion_FederatedLearning/20230410_实验1_0.8_8_cifar10.png')
    plt.show()
    ...
    plt.legend()
    plt.savefig(
        'D:/Code/WorkPlace/pythonWork/FL_MultiTask/save/Different_Budget_EMD_Random_comparsion_FederatedLearning/20230410_实验1_0.8_8_算法平均精度.png')
    plt.show()
