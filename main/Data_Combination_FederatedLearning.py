...
from util.client_selection import getClientBudget, getClientBudget_q, CLQM,bidPriceFirst_ClientSelection, RandomClientSelect
from util.getClientQuality import getQ
from util.dirichlet import getTestDistribution, distribute_data_dirichlet, \
    preferDistribution, exchangeDistribution
...
def fl_training(idxs_users, dict_users, p, dataset_train, dataset_test, args, task, class_weight1):
    ...
    elif args.model == 'mlp' and task == 'fmnist':
        print("cnn训练")
        net_glob = CNNMnist(args=args).to(args.device)
    elif args.model == 'mlp':
        print("mlp训练")
...
def main():
    ...
    # 狄利克雷获取所有客户端的本地数据分布(所有任务的)
    dict_users_mnist1, pic_distribution_every_client_mnist1, _, _ = distribute_data_dirichlet(train_dataset_mnist, args, n_class=10)
    dict_users_fmnist1, pic_distribution_every_client_fmnist1, _, _ = distribute_data_dirichlet(train_dataset_fmnist, args, n_class=10)
    dict_users_cifar101, pic_distribution_every_client_cifar101, _, _ = distribute_data_dirichlet(train_dataset_cifar10, args, n_class=10)

    # 极端情况下获取所有客户端的本地数据分布(所有任务的)
    dict_users_mnist2, distribution_mnist2,class_weight_mnist = preferDistribution(train_dataset_mnist, args)
    ...
    dict_users_fmnist2, distribution_fmnist2,class_weight_fmnist = exchangeDistribution(dict_users_fmnist, distribution_fmnist, 'fmnist')
    dict_users_cifar102, distribution_cifar102,class_weight_cifar10 = exchangeDistribution(dict_users_cifar10, distribution_cifar10, 'cifar10')

    client_dict = []

    #mnist
    dict_users_mnist = defaultdict(list)
    for i in range(len(dict_users_mnist1)):
        test = np.hstack((dict_users_mnist1[i], dict_users_mnist2[i]))
        dict_users_mnist[i] = test.tolist()


    client_dict_mnist = pic_distribution_every_client_mnist1.copy()
    for i in range(len(pic_distribution_every_client_mnist1)):
        for j in range(len(pic_distribution_every_client_mnist1[i])):
            client_dict_mnist[i][j] += distribution_mnist2[i][j]
    client_dict.append(client_dict_mnist)

    #fmnist
    dict_users_fmnist = defaultdict(list)
    for i in range(len(dict_users_fmnist1)):
        test = np.hstack((dict_users_fmnist1[i], dict_users_fmnist2[i]))
        dict_users_fmnist[i] = test.tolist()

    client_dict_fmnist = pic_distribution_every_client_fmnist1.copy()
    for i in range(len(pic_distribution_every_client_fmnist1)):
        for j in range(len(pic_distribution_every_client_fmnist1[i])):
            client_dict_fmnist[i][j] += distribution_fmnist2[i][j]
    client_dict.append(client_dict_fmnist)

    #cifar10
    dict_users_cifar10 = defaultdict(list)
    for i in range(len(dict_users_cifar101)):
        test = np.hstack((dict_users_cifar101[i], dict_users_cifar102[i]))
        dict_users_cifar10[i] = test.tolist()

    client_dict_cifar10 = pic_distribution_every_client_cifar101.copy()
    for i in range(len(pic_distribution_every_client_cifar101)):
        for j in range(len(pic_distribution_every_client_cifar101[i])):
            client_dict_cifar10[i][j] += distribution_cifar102[i][j]
    client_dict.append(client_dict_cifar10)
    ...
    '''——————————————————————报价优先选择的客户端——————————————————————'''
    BidPrice_first_Selected, BidPrice_first_Selected_Payment = bidPriceFirst_ClientSelection(budget_q_2,Budget_Task)


    '''——————————————————————随机选择的客户端——————————————————————'''
    Random_Selected,Random_Selected_Payment = RandomClientSelect(budget_q_2,Budget_Task,args)
    ...
    for task in Task:
        ...
        if task == 0:
            acc_test, acc_all_mnist = fl_training(client_selected_every_task_q[task], dict_users_mnist, p, dataset_train, dataset_test, args, dataset_name[task],class_weight_mnist)
    ...
    # for i in range(len(X)):
    #     Acc_avg_EMD_MQFL.append((acc_all_mnist[i] + acc_all_cifar10[i])/2)
    #     Acc_avg_BidPrice_first.append((BidPrice_first_acc_all_mnist[i] + BidPrice_first_acc_all_cifar10[i]) / 2)
    #     Acc_avg_Random.append((Random_acc_all_mnist[i]+Random_acc_all_cifar10[i])/ 2)
    ...
    plt.grid()
    plt.legend()
    plt.savefig('D:/Federated_Learning_experiments/FL_MultiTask/save/Task3_experiment_conclusion/20230228_实验2_0.5_3_TaskQuality.png',dpi=600)
    plt.show()
    ...
    plt.grid()
    plt.legend()
    plt.savefig('D:/Federated_Learning_experiments/FL_MultiTask/save/Task3_experiment_conclusion/20230228_实验2_0.5_3_mnist.png',dpi=400)
    plt.show()
    ...
    plt.grid()
    plt.legend()
    plt.savefig('D:/Federated_Learning_experiments/FL_MultiTask/save/Task3_experiment_conclusion/20230228_实验2_0.5_3_fmnist.png', dpi=400)
    plt.show()
    ...
    plt.grid()
    plt.legend()
    plt.savefig('D:/Federated_Learning_experiments/FL_MultiTask/save/Task3_experiment_conclusion/20230228_实验2_0.5_3_cifar10.png', dpi=400)
    plt.show()
    ...
    plt.grid()
    plt.legend()
    plt.savefig('D:/Federated_Learning_experiments/FL_MultiTask/save/Task3_experiment_conclusion/20230228_实验2_0.5_3_算法平均精度.png',dpi=400)
    plt.show()
